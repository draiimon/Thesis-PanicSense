...
interface AnalyzedText {
  ...
  sentimentCategory?: string; // General sentiment: Positive, Negative, Neutral
  confidence: number;
  timestamp: Date;
  language: string;
  ...
  updatedAt?: string;
  generalSentiment?: string; // Legacy field for backward compatibility
}

export function SentimentMessage({ item }: { item: AnalyzedText }) {
  return (
    <div className="relative">
      ...
      <div className="relative flex items-start">
        {/* Large Emoji Profile Picture */}
        <div className="mr-3 flex-shrink-0">
          <div className="w-14 h-14 rounded-full flex items-center justify-center text-4xl bg-gradient-to-br from-indigo-100 to-purple-100 border border-indigo-200/80 shadow-md">
            {/* Emotion Sentiments */}
          ...
          <div className="p-3 bg-white/80 backdrop-blur-sm rounded-lg border border-slate-200/60 shadow-sm">
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center gap-2">
                ...
                {/* Add general sentiment badge - shows Positive/Negative/Neutral */}
                {(item.sentimentCategory || item.generalSentiment) && (
                  <Badge 
                    className={`${
                      (item.sentimentCategory || item.generalSentiment) === "Positive"
                        ? "bg-green-100 text-green-700 border-green-200" 
                        : (item.sentimentCategory || item.generalSentiment) === "Negative"
                  ...
                  >
                    {(item.sentimentCategory || item.generalSentiment) === "Positive" && "🙂 "}
                    {(item.sentimentCategory || item.generalSentiment) === "Negative" && "🙁 "}
                    {(item.sentimentCategory || item.generalSentiment) === "Neutral" && "😐 "}
                    {item.sentimentCategory || item.generalSentiment}
                  </Badge>
                )}
